...
export default function Admins({ role }) {
  ...
  async function handleUpdatePassword(e) {
    ...
    if (newPassword.length < 6) { setPassMsg({ type: 'error', text: 'Password must be at least 6 characters long.' }); return; }
  ...
  async function handleAddAdmin(e) {
    ...
    if (error) setAdminMsg({ type: 'error', text: 'Failed to add. This email might already be an admin.' });
    else {
  ...
  async function handleRemoveAdmin(id, email) {
    ...
    if (!error) setAdminsList(adminsList.filter(a => a.id !== id));
    else alert("Failed to remove admin: " + error.message);
  }
      ...
      <div className="card p-0" style={{ overflow: 'hidden' }}>
        ...
        <div style={{ padding: '32px' }}>
          ...
          <div style={{ border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden' }}>
            {isLoadingAdmins ? <p className="text-secondary text-center p-24 m-0">Loading...</p> : adminsList.length > 0 ? (
                ...
                {adminsList.map((admin, index) => (
                  <div key={admin.id} className="flex-between align-center" style={{ padding: '16px 20px', borderBottom: index !== adminsList.length - 1 ? '1px solid var(--border-color)' : 'none' }}>
                    ...
                    {admin.email !== 'admin@hearable.com' ? (
                      <button onClick={() => handleRemoveAdmin(admin.id, admin.email)} style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', fontWeight: '500', fontSize: '0.9rem' }}>Revoke Access</button>
