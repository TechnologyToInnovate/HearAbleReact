...
export default function CertificateModal({ isOpen, onClose, userId, onSuccess, initialData }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        if (initialData.certificate_skills) {
          const loadedSkills = initialData.certificate_skills.map(ws => ws.skills);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      let certId = initialData?.id;

      if (certId) {
        await supabase.from('certificates').update({ title, description, file_url: finalFileUrl }).eq('id', certId);
        await supabase.from('certificate_skills').delete().eq('certificate_id', certId);
      } else {
      ...
      if (selectedSkills.length > 0) {
        ...
        await supabase.from('certificate_skills').insert(skillInserts);
      }
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div className="card p-32" style={{ width: '100%', maxWidth: '600px', overflow: 'visible' }}>
        <h2 className="m-0 mb-24 text-2xl">{initialData ? 'Edit' : 'Add'} Certificate</h2>
        
        <form onSubmit={handleSave} className="flex-col gap-16">
          <div>
            <label className="font-bold text-sm block mb-8">Certificate Title *</label>
